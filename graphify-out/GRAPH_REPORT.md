...
1. `Select()` - 53 edges
2. `Base` - 49 edges
3. `AppException` - 32 edges
4. `User` - 31 edges
5. `refresh()` - 30 edges
6. `_to_out()` - 23 edges
7. `RequestIdMiddleware` - 20 edges
8. `Project` - 20 edges
...
- **Are the 47 inferred relationships involving `Base` (e.g. with `Ensure all database tables exist on startup.     Uses create_all with checkfirs` and `AcademicYear`) actually correct?**
  _`Base` has 47 INFERRED edges - model-reasoned connections that need verification._
- **Are the 29 inferred relationships involving `AppException` (e.g. with `Ensure all database tables exist on startup.     Uses create_all with checkfirs` and `AttendanceService`) actually correct?**
  _`AppException` has 29 INFERRED edges - model-reasoned connections that need verification._